...
if df_goster is not None:
    ...
    with st.expander("Filtrele", expanded=False):
        ...
        sinyal_sutunlari = [
            ...
            "Kombine_Dip", "SSL&EMA_Sinyal", "BB_Sikisma","BB_Fiyat_Durum","Destek_Direnc"
        ]
        ...
        sayisal_sutunlar = [c for c in ["BB_Bant_Durumu","EMA_Durumu", "RSI", "StochRSI", "TSI"] if c in df_filtreli.columns]
    ...
    st.dataframe(df_filtreli, use_container_width=True, column_config={"EMA_Durumu": st.column_config.NumberColumn(format="%.2f%%")})
